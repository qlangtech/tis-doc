...
export const VerShell = () => {
  return (
    <div>
      <CodeBlock
        language="shell">
        wget http://mirror.qlangtech.com/{metadata.version}/tis/tis-uber.tar.gz
      </CodeBlock>
    </div>
  );
}
...
2. `Java SE Development Kit 8`[安装](https://www.oracle.com/java/technologies/javase/javase-jdk8-downloads.html)
