...
* **易与大数据生态整合**
  ...
  支持与DolphinScheduler整合，用户只需在TIS中配置数据管道，验证无误后，一键推送同步表任务到DolphinScheduler平台，即可在其上完成数据同步任务调度。为以DolphinScheduler构建ETL数仓方案如虎添翼。<Link href={require("@site/docs/guide/integer-dolphinscheduler/index.mdx")} >详细说明</Link>
