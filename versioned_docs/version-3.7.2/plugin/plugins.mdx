...
4. index
	...
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@3c9ef6e9
...
4. column
	...
	* **说明:** 
		 ...
		 * 类型转换
		 ...
		 | Boolean  | boolean |
		 | Bytes    | bytes |
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@1ab14636
...
5. collectionName
	...
	* **说明:** MonogoDB的集合名。【必填】
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@16b3c905
...
2. uniqueKey
	...
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@2f9a10df
...
3. uniqueKey
	...
	* **说明:** 
		...
		* 注意：如果此参数为空，并且写入模式为 update 和 replace 时，应用会自动获取数据库中的唯一索引；
		如果数据表没有唯一索引，但是写入模式配置为 update 和 replace，应用会以 insert 的方式写入数据；
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@78d23d6a
...
2. uniqueKey
	...
	* **说明:** 
		...
		* 注意：如果此参数为空，并且写入模式为 update 和 replace 时，应用会自动获取数据库中的唯一索引；
		如果数据表没有唯一索引，但是写入模式配置为 update 和 replace，应用会以 insert 的方式写入数据；
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@4b41587d
