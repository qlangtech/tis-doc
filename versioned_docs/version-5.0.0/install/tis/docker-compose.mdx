...
let configPropUrl = 'https://github.com/datavane/tis/blob/v' + metadata.version + '/docker-compose/config/config.properties' ;
 let projectUrl = 'https://github.com/datavane/tis/blob/v' + metadata.version;
return <ul> <li> 所在Github项目： <a href={projectUrl}>{projectUrl}</a> </li> <li> docker-compose.yaml文件路径：<a href={fileUrl}>{fileUrl} </a></li> <li> config.properties文件路径：<a href={configPropUrl}>{configPropUrl} </a></li></ul>
}

在单机上通过`docker-compose`方式启动TIS集群。集群包含两个服务组件：
1. 增量实时同步组件`Standalone Flink`
2. TIS控制台与执行批量同步的`tis-console`

## 环境要求
1. docker 版本：`Docker version 24.0.5, build ced0996`
2. docker-compose 版本：`Docker Compose version v2.29.6`

## 配置
使用docker-compose启动TIS集群需要在本地环境中先做一些配置，需要准备以下这些资源
...
### 2.配置 config/config.properties
在 `docker-compose.yaml` 配置文件所在的相同目录中 创建 `config/config.properties` 配置文件,内容如下：
``` properties title='config.properties' showLineNumbers
project.name=TIS
runtime=daily
...
### 3.初始化data目录
在 `docker-compose.yaml` 配置文件所在相同目录中创建 `data`目录
``` shell
mkdir -p data
```
