...
启动入口为JUnit单元测试类StartTISWeb，位置在`tis/tis-console/src/test/java/`，需要运行按如下步骤：

1. `cd tis/tis-console`
2. 创建初始化token文件：
   ```shell
...
3.  运行
   macBook Intel 芯片
   ```shell
     npm run ng:serve-jit-amd64 --scripts-prepend-node-path=auto
   ```
   macBook ARM 芯片
   ```shell
     npm run ng:serve-jit-arm64 --scripts-prepend-node-path=auto
   ```
